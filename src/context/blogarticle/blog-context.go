...
At Google, we developed a context package that makes it easy to pass request-scoped values, cancelation signals, and deadlines across API boundaries to all the goroutines involved in handling a request. The package is publicly available as golang.org/x/net/context. This article describes how to use the package and provides a complete working example.
...
WithCancel and WithTimeout return derived Context values that can be canceled sooner(更快) than the parent Context. The Context associated with an incoming request is typically canceled when the request handler returns. WithCancel is also useful for canceling redundant requests(比如同时查询多个从库) when using multiple replicas. WithTimeout is useful for setting a deadline on requests to backend servers(比如限制db查询时间):

// WithCancel returns a copy of parent whose Done channel is closed as soon as
// parent.Done is closed or cancel is called.
// WithCancel会返回parent的一个拷贝,对于返回的这个ctx Context
// 一旦parent.Done被关闭的时候,或者cancel被调用的时候,ctx.Done也会被关闭
...
// any. If the timer is still running, the cancel function releases its resources.
...
func httpDo(ctx context.Context, req *http.Request, f func(*http.Response, error) error) error {
    ...
    c := make(chan error, 1)
    go func() { c <- f(client.Do(req)) }() // 启动一个线程来进行http请求
    select {
    case <-ctx.Done():
        // 操作被cancel
        tr.CancelRequest(req)
        <-c // Wait for f to return.
        return ctx.Err() // ctx.Err(): 被取消的原因
    case err := <-c:
        return err // 返回 f 运行的结果
    }
}
